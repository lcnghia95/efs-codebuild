...
//   let naviProductIds = _filterIds(products, NAVI_PRODUCT_TYPE_ID)
//   if (naviProductIds.length == 0) {
//     return {}
//   }

//   let [seriesObjects, articleObjects] = await Promise.all([
...
//   let eventProductIds = _filterIds(products, EVENT_PRODUCT_TYPE_ID)
//   if (eventProductIds.length == 0) {
//     return {}
//   }
//   let events = await _relatedData(eventProductIds, app.models.Events)
...
//   let videoProductIds = _filterIds(products, VIDEO_PRODUCT_TYPE_ID)
//   if (videoProductIds.length == 0) {
//     return {}
//   }
//   let videos = await _relatedData(videoProductIds, app.models.VideoRankingAccess)
...
function _url(product, systemtradeCategoryMap, naviMap = [], eventIdMap = [], videoIdMap = []) {
  ...
  if (typeId == 1) {
    ...
    if (categoryId == FX_PRODUCT_CATEGORY_ID) {
      return '/vi/systemtrade/fx/' + productId
    }
    if (categoryId == STOCK_PRODUCT_CATEGORY_ID) {
      return '/vi/systemtrade/stocks/' + productId
    }
  ...
  if (typeId == 3) {
    if ((naviMap[productId] || {}).flag == SERIES_FLAG) {
      ...
      return `/finance/navi/${naviMap[productId].seriesId}/${naviMap[productId].id}`
    }
    return UNDEFINED_URL
  }
...
async function productDetailUrls(products) {
  ...
  return products.reduce((result, product) => {
    result[product.id || product.productId] =
      _url(product, systemtradeCategoryMap)
