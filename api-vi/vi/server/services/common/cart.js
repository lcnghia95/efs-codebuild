...
async function show(pId, data = {}, userId = 0, isPurchased = 0) {
  if (data) {
    ...
    if (data) {
      if(data.productNameVi){
        data.productName = data.productNameVi
      } else {
        slackCommonService.sendSlackLanguages(productDetailModel, pId, 'productId')
      }
    }
  }
...
function price(data) {
  ...
  if (!data.isSpecialDiscount || (!count && !start && !end)) {
    return {
      price: data.price,
      priceVi: parseInt(data.price * rate),
    }
  }
