...
async function show(id, input, meta) {
  if (!id || !await helper.validateCategoryId(id, STOCK_CATEGORY_ID)) {
    return {}
  }
  ...
  const [systemtrade, questionTotal, cartInfo, symbol] = await Promise.all([
    _systemtrade(id, product.platformId),
  ...
  // get EN name from product
  cartInfo.name = product.productName
...
function _outlineObject(product, symbol) {
  return {
    ...
    symbolName: symbol,
    outline: stringUtil.externalLink(product.productOutline) || '',
  }
}
...
function _criterialObject(data) {
  let winRate = data.PercentProfitable || data.AllPercentProfitable || 0,
    profitFactor = data.ProfitFactor || data.AllProfitFactor || 0
