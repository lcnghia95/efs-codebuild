...
const SALONS_PRODUCT_TYPE_ID = 4
 ...
 * @private
 */
async function _salonsMap(products) {
  const salonsProductIds = _filterIds(products, SALONS_PRODUCT_TYPE_ID)
  if (salonsProductIds.length == 0) {
    return {}
  }

  const salons = await _relatedData(salonsProductIds, app.models.Salons)

  return arrayUtil.index(salons, 'productId', 'id')
}
 ...
 * @private
 */
function _url(product, systemtradeCategoryMap, naviMap = [], eventIdMap = [], videoIdMap = [], salonsMap = []) {
  ...
  // Salons
  if (typeId == SALONS_PRODUCT_TYPE_ID) {
    if (IGNORE_PRODUCT_IDS.includes(productId)) {
      return '/finance/salons/' + productId
    } else {
      return '/finance/mailmagazine/' + salonsMap[productId]
    }
  }
...
async function productDetailUrls(products) {
  ...
  return products.reduce((result, product) => {
    result[product.id || product.productId] =
      _url(product, systemtradeCategoryMap, naviMap, eventIdMap, videoIdMap, salonsMap)
    return result
  }, {})
}
