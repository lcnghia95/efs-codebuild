    ...
    async asyncData({ app, params, error, req, store }) {
      let id = params.id,
        ...
        [toolData, externalUrl, isRangePrice, realtradeWidgets] = await Promise.all([
          app.GoGoHTTP.get(`/api/v3/surface/tools/${id}?type=ebooks`),
      ...
      store.commit('pushBC', {
        name: data.name,
        target: { name: 'tools-ebooks-id', params: params },
      })
      const { 
      ...
      return {
        ...
        linkMeta: [
          {
            rel: 'canonical',
            href: `https://www.gogojungle.co.jp/tools/ebooks/${id}`,
          },
        ],
        jsonLDTemplate: generateLDJson({
          ...
          url: `${process.env.GOGO_URL}/tools/ebooks/${id}`,
    ...
    methods: {
      ...
      getProductPassword(password) {
        return this.GoGoHTTP.post(
          ...
          { pagePassword: password, type: 'ebooks' }
        ).then(data => {
