...
const {_appendSystemtrade} = require('@@server/services/common/product')
...
async function show(id, input = {}) {
  const data = await _getProduct(id, input)

  // Add set products in case is_set = 1
  if (data.isSet === 1) {
    data.setProducts = await _setProducts(data.subProductIds)
  }
  return data
}
...
async function _cachePassword(id, password, requestId) {
  return new Promise(((resolve, reject) => {
    const success = app.emit(
      'setHeader' + requestId,
      'Set-Cookie',
      cookieModule.serialize(_key(id), cryptoUtil.encrypt(
        password), {
...
async function buildRelationalFxData(sfProduct, input, userId) {
  const [systemtrade, forward, questionTotal, isFavorite, isPortfolio] = await Promise.all([
    _appendSystemtrade(sfProduct.id),
    _appendForward(sfProduct.id, commonUser.oldDeveloperId(sfProduct.userId)),
...
async function buildUpInsertFxData(product, input, userId, isEmptySfpd, languageProduct) {
  let buildProduct = {
    id: product.id,
    isSync: 1, // Mark sync
  ...
  await _cloneProductInfo(buildProduct, product, languageProduct)
  // [systemtrade, forward, questionTotal, isFavorite, isPortfolio]
  const [systemtrade, forward, questionTotal, isFavorite, isPortfolio] = await Promise.all([
      _appendSystemtrade(product.id),
      _appendForward(product.id, commonUser.oldDeveloperId(product.userId)),
  ...
  if (isEmptySfpd) {
    sfProductDetailModel.create(pdMdObj)
  } else {
    sfProductDetailModel.upsert(pdMdObj)
  }
...
async function _appendOutline(object) {
  ...
  object.productOutlineVi = (outline.outline || '')
...
async function _cloneProductInfo(object, product, languageProduct) {
  ...
  object.productNameVi = languageProduct.name
