...
async function data(pId, year) {
  ...
  const rawData = !product ? {} : await _rawData(pId, product.userId, product.platformId, year)
...
async function getStockData(product, year) {
  ...
  FROM ${table}
  WHERE
    IsValid = 1 AND CloseTime >= (?) AND CloseTime < (?)
  ...
  for (let idx = 0; idx < 12; idx++) {
    ...
    if(objUtil.isPresent(monthData)) {
...
function _table(pId, oldUId = 0, platform = 0) {
  ...
  return (subPlatform[platform] || '') +
    sprintf('%04d', pId) + '_' +
    sprintf('%04d', oldUId) + '_' + '0000'
}
...
function _object(rawData) {
  ...
  for (let idx = 0; idx < 12; idx++) {
    ...
    pf[idx] = arrayUtils.sum(monthly['profit'])
