...
async function onSelect(id, meta) {
  ...
  const data = await portfolioSelectionProductsModel.create({
    isValid: 1,
    userId: meta.userId,
    productId: id,
  })
...
async function onRemove(pId, meta) {
  ...
  const data = await portfolioSelectionProductsModel.findOne({
    where: {
      isValid: 1,
      userId: meta.userId,
      productId: pId,
  ...
  data.destroy()
  const fxData = await modelUtils.findOne('portfolio', 'product', {
    where: {
      id: data.id,
    },
  })
