<template>
  <div id="community-area">
    <div class="community-post p-20">
      ...
      <textarea class="text-area fs-12 w-full resize-vertical p-10 mt-5" v-model="commentText" :placeholder="$t('3')" maxlength="2000" />
        ...
        <div class="showreview modal fade" role="dialog" v-if="$store.state.user.id">
          <div class="modal-dialog flex layout-col">
            ...
            <div class="review-content">
              ...
              <SystemUserMessage :buyer="communities.buyer" :message="reviewMsg" :is-preview="true" :disable-reply="true"/>
    ...
    <div v-if="!loading" class="communities mt-30 p-20" id="comment-info">
      ...
      <div class="search-content flex mid center" @keyup.enter="onSearch">
      ...
      <SystemUserMessage v-for="commu in reviewData.data" :message="commu" :key="'commu-'+commu.id" class="mt-20 mb-40" :on-delete-topic="onDeleteTopic" :on-delete-reply ="onDeleteReply" @posted="onPosted"/>
      <div class="text-center mt-20">
        <paging :cur-page="reviewData.currentPage"
                :total="reviewData.lastPage"
                :from="reviewData.pagingFrom"
    ...
    <GogoModal :show="isShowModal" :cancel-text="$t('16')" @close="onClose" @ok="onOkDeleteTopic" :ok-text="'OK'" :width="'400px'" :height="'auto'">
...
import SystemUserMessage from './SystemUserMessage.vue'
...
export default {
  ...
  created() {
    this.$eb.$on(this.$ebc.SYSTEMTRADE_GO_COMMUNITY, e => {
      this.initFirstData(e)
    })
  },
  methods: {
    initFirstData(gotoId) {
      this.getData(data => {
        this.loading = false
        this.communities = data
        this.data = this.communities.comments
        this.topicNum = this.topicNum || data.topic || 0
        if (gotoId && typeof  gotoId == 'string') {
          ...
          if (parseInt(gotoId[1])) {
            ...
            this.$nextTick(() => {
              $('html, body').animate(
                {
                  scrollTop: $('#commu-' + gotoId[0]).offset().top - 30,
                },
                'slow'
              )
            })
          ...
          this.reviewData = calPaging(this.data, 1, 10, 4)
          $('#detail-tab').length && $('html, body').animate(
            {
              scrollTop: $('#detail-tab').offset().top - 10,
            },
            'slow'
          )
        }
    ...
    onOkDeleteTopic() {
      ...
      this.GoGoHTTP.delete('/api/v3/surface/communities/' + this.keepId).then(() => {
    ...
    onDeleteReply(id) {
      return this.GoGoHTTP.delete(`/api/v3/surface/communities/${id}`).then(() => {
        this.$emit('delete-reply')
      })
    },
    onOK() {
      this.keepFn && this.keepFn(this.keepId)
    },
    onClose() {
      this.isShowModal = false
    }
