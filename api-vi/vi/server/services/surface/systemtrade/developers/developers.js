...
async function index(input) {
  ...
  const where = objectUtil.nullFilter({
    isValid: 1,
    nickName: !name ? null : {
      like: '%' + name + '%',
    },
  })
  const users = await pickupDevelopersModel.find({
    where,
...
async function _users(limit) {
  const users = await rankingBestSellersModel.find({
    where: {
      isValid: 1,
    },
    limit,
...
async function top() {
  ...
  const data = slice(shuffle(users), 0, 5)
...
async function newTop(number = 5) {
  ...
  while (loop) {
    const tempUsers = await _getUsers(limit, skip)
  ...
  const data = slice(shuffle(users), 0, number)
