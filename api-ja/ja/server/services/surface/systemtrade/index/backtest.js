...
const {removeRedundantInfo} = require('@@server/services/surface/backtest/helper')
...
const DEFAULT_MESSAGES = {
  ...
  4: 'Hiện tại chúng tôi không thể hiển thị backtest cho sản phẩm này. Xin lỗi vì sự bất tiện này!',
  5: '无法表示回测资料。',
  6: '無法表示回測資料。',
}
...
async function backtestIndex(productId, query, userId = 0) {
  ...
  const data = backtestList.filter(e => users[e.userId] && e.userId != productData['userId'] && e.userId != 110001).map(el => {
    el.nickName = users[el.userId].nickName || ''
    return el
  })
  const dataGGJ = backtestList.filter(e => users[e.userId] && e.userId == 110001).map(el => {
    el.nickName = users[el.userId].nickName || ''
    return el
  })
