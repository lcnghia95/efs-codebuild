...
async function action(cId, type, meta) {
  ...
  const communityGood = await communityGoodModel.create({
    isValid: 1,
    communityId: cId,
    userId: meta.userId,
    goodType: type,
 ...
 * @public
 */
async function lastest(pId, input) {
  pId = parseInt(pId)
...
async function count(productId) {
  return productId ? await communityModel.count({
    isPrivate: 0,
    productId,
    userId: {
      gt: 0,
...
async function destroy(id, userId) {
  ...
  const record = await communityModel.findOne({
    where: {
      id,
      isValid: 1,
      userId,
 ...
 * @private
 */
async function _getCommunitiesData(pId, offset) {
  return communityModel.find({
    where: {
      isValid: 1,
      isPrivate: 0,
