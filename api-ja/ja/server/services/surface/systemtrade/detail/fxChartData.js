...
async function _getProfit(siTable, year) {
  ...
  (
    SELECT 
      ...
      SUM(CASE WHEN ProfitTotal > 0 AND Position = '${reversePositionMap.buy}' THEN ProfitTotal ELSE 0 END) AS buyProfits,
      SUM(CASE WHEN ProfitTotal > 0 AND Position = '${reversePositionMap.buy}' THEN 1 ELSE 0 END) AS buyProfitsLength,
      SUM(CASE WHEN ProfitTotal <= 0 AND Position = '${reversePositionMap.buy}' THEN ProfitTotal * -1 ELSE 0 END) AS buyLosses,
      SUM(CASE WHEN ProfitTotal <= 0 AND Position = '${reversePositionMap.buy}' THEN 1 ELSE 0 END) AS buyLossesLength,
      SUM(CASE WHEN ProfitTotal > 0 AND Position = '${reversePositionMap.sell}' THEN ProfitTotal ELSE 0 END) AS sellProfits,
      SUM(CASE WHEN ProfitTotal > 0 AND Position = '${reversePositionMap.sell}' THEN 1 ELSE 0 END) AS sellProfitsLength,
      SUM(CASE WHEN ProfitTotal <= 0 AND Position = '${reversePositionMap.sell}' THEN ProfitTotal * -1 ELSE 0 END) AS sellLosses,
      SUM(CASE WHEN ProfitTotal <= 0 AND Position = '${reversePositionMap.sell}' THEN 1 ELSE 0 END) AS sellLossesLength,
      SUM(CASE WHEN Position = '${reversePositionMap.buy}' THEN 1 ELSE 0 END) AS buyCount,
      SUM(CASE WHEN Position = '${reversePositionMap.sell}' THEN 1 ELSE 0 END) AS sellCount
    FROM ${siTable}
    WHERE isOpen = 0 AND CloseDate >= (?) AND CloseDate < (?)
    GROUP BY MONTH(CloseDate)
  ) AS t1`
  return (await modelUtils.excuteQuery('fx_system', sql, [`${year}-01-01 00:00:00z`, `${year + 1}-01-01 00:00:00z`])) || []
}
...
async function _getPair(siTable, year) {
  const sql = `SELECT
    closeMonth,
    pair,
    COALESCE(buyDuration / NULLIF(buyDurationCount,0), 0) AS bPosition,
  ...
  ( 
    SELECT
      MONTH(CloseDate) AS closeMonth,
      Pair AS pair,
      SUM(CASE WHEN Position = '${reversePositionMap.buy}' THEN TIMESTAMPDIFF(SECOND, CheckDate, CloseDate)/3600 ELSE 0 END) AS buyDuration,
      SUM(CASE WHEN Position = '${reversePositionMap.buy}' THEN 1 ELSE 0 END) AS buyDurationCount,
      SUM(CASE WHEN Position = '${reversePositionMap.sell}' THEN TIMESTAMPDIFF(SECOND, CheckDate, CloseDate)/3600 ELSE 0 END) AS sellDuration,
      SUM(CASE WHEN Position = '${reversePositionMap.sell}' THEN 1 ELSE 0 END) AS sellDurationCount
    FROM ${siTable}
    WHERE isOpen = 0 AND CloseDate >= (?) AND CloseDate < (?)
...
async function _monthlyData(pId, inputYear) {
  ...
  return arrayUtils.index(await modelUtils.find('fx_system', 'ns_collect_monthly', {
    where: {
      ProductID: pId,
      Year: inputYear,
    },
...
function _object(rawData, monthlyData) {
  ...
  for (let idx = 0; idx < 12; idx++) {
    ...
    const bf = bfCount > 0 ? arrayUtils.sum(buyProfit) / bfCount : 0
    const bl = blCount > 0 ? arrayUtils.sum(buyLoss) / blCount : 0
    ...
    const sf = sfCount > 0 ? arrayUtils.sum(sellProfit) / sfCount : 0
    const sl = slCount > 0 ? arrayUtils.sum(sellLoss) / slCount : 0
    ...
    Object.keys(buyDuration).forEach(key => {
      ...
      bPosition.push(
        !buyDuration[key].length ?
          0 :
          (arrayUtils.sum(buyDuration[key]) / buyDuration[key].length)
            .toFixed(2),
      )
      ...
      sPosition.push(
        !sellDuration[key].length ?
          0 :
          (arrayUtils.sum(sellDuration[key]) / sellDuration[key].length)
            .toFixed(2),
      )
    })
...
function _buildObject(profitData, pairData, monthlyData) {
  ...
  for (let idx = 0; idx < 12; idx++) {
    ...
    if(objUtil.isPresent(mProfitData)) {
    ...
    if(objUtil.isPresent(mPairData)) {
